...
# Function for calculating classification errors within bins:
# The x column identified by the 'which' argument
# contains the error values to be averaged.
means0 <- function(x, which = "acc", breaks = 25) {
  intv <- cut(x$dist, breaks = breaks)
  RES <- data.frame(
    dist = unname(tapply(x$dist, intv, median))
  )
  mymean <- function(lev, WHICH) mean(x[intv==lev, WHICH], na.rm = TRUE)
  for (wh in which) {
    means <- levels(intv) %>% map(mymean, WHICH = wh) %>% unlist()
    RES[[wh]] <- unname(means)
  }
  RES
}
...
# Hybrid ordinary kriging - linear model
oklm <- function(formula, data, 
                 locations = ~x+y, 
                 svgm = "Sph", range = NA,
                 nsratio = 0.25, cressie = TRUE,
                 fixed = FALSE, fit.ranges = TRUE, nmax = Inf,
  ...
  data$TEMPRESID <- residuals(lmfit)
  predictors <- all.vars(formula)[-1]
  okformula <- as.formula("TEMPRESID ~ 1")
  
  # Fit ordinary kriging model:
  okfit <- mykrigeOK(formula = okformula, data = data, locations = locations,
                     svgm = svgm, range = range, nsratio = nsratio,
                     cressie = cressie, fixed = fixed, fit.ranges = fit.ranges,
                     nmax = nmax)
...
# Predict method for fitted okrf models (OK-RF):
predict.oklm <- function(object, newdata, nmax = object$okfit$nmax, ...) {
  okpred <- predict(object = object$okfit, newdata = newdata, nmax = nmax)
  lmpred <- predict(object = object$lmfit, newdata = newdata)
  pred <- lmpred + okpred
  pred
}
